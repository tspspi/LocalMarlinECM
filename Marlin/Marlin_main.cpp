 ...
 *  M86 - Switching additional PSU for heated bed ON (on parameter selecting the PSU)
 *  M87 - Switching additional PSU for heated bed OFF (on parameter selecting the PSU)
 *  M88 - Getting status for additional PSU for heated bed (on parameter selecting the PSU)
...
// Extruder offsets
  ...
  bool psuMultiOn[HAS_MULTIPLE_ADDITIONAL_PSU] = {
    #if HAS_MULTIPLE_ADDITIONAL_PSU > 0
      false
    #endif
    #if HAS_MULTIPLE_ADDITIONAL_PSU > 1
      false
    #endif
  };
#endif
...
inline void gcode_M140() {
  if (DEBUGGING(DRYRUN)) return;
  #if defined(HAS_MULTIPLE_ADDITIONAL_PSU) && (HAS_MULTIPLE_ADDITIONAL_PSU > 0) && defined(MULTIPSU_PSU1_PIN) && defined(MULTIPSU_AUTOENABLE_BED_PSU1)
    if(parser.value_celsius() > 0) {
      if(!psuMultiOn[0]) {
        OUT_WRITE(MULTIPSU_PSU1_PIN, MULTIPSU_ON);
        psuMultiOn[0] = true;
      }
    } else {
      if(psuMultiOn[0]) {
        OUT_WRITE(MULTIPSU_PSU1_PIN, MULTIPSU_SLEEP);
        psuMultiOn[0] = false;
      }
    }
  #endif
...
inline void gcode_M81() {
  ...
  #ifdef HAS_MULTIPLE_ADDITIONAL_PSU
      #if (HAS_MULTIPLE_ADDITIONAL_PSU > 0) && defined(MULTIPSU_PSU1_PIN)
        OUT_WRITE(MULTIPSU_PSU1_PIN, MULTIPSU_SLEEP);
        psuMultiOn[0] = false;
      #endif
      #if (HAS_MULTIPLE_ADDITIONAL_PSU > 1) && defined(MULTIPSU_PSU2_PIN)
        OUT_WRITE(MULTIPSU_PSU2_PIN, MULTIPSU_SLEEP);
        psuMultiOn[1] = false;
      #endif
  #endif
  ...
  inline void gcode_M86() {
    if (parser.seen('A')) {
      ...
      #if (HAS_MULTIPLE_ADDITIONAL_PSU > 1) && defined(MULTIPSU_PSU2_PIN)
        OUT_WRITE(MULTIPSU_PSU2_PIN, MULTIPSU_ON);
        psuMultiOn[1] = true;
      #endif
    }
  }
  ...
  inline void gcode_M88() {
    if (parser.seen('A')) {
      #if (HAS_MULTIPLE_ADDITIONAL_PSU > 0) && defined(MULTIPSU_PSU1_PIN)
        serialprintPGM(psuMultiOn[0] ? PSTR("PS:1\n") : PSTR("PS:0\n"));
      #endif
    } if (parser.seen('B')) {
      #if (HAS_MULTIPLE_ADDITIONAL_PSU > 1) && defined(MULTIPSU_PSU1_PIN)
        serialprintPGM(psuMultiOn[1] ? PSTR("PS:1\n") : PSTR("PS:0\n"));
      #endif
    }
  }
#endif
...
void setup() {
  ...
  #if defined(HAS_MULTIPLE_ADDITIONAL_PSU)
    #if HAS_MULTIPLE_ADDITIONAL_PSU > 0
      SET_OUTPUT(MULTIPSU_PSU1_PIN);
      OUT_WRITE(MULTIPSU_PSU1_PIN, HIGH);
      psuMultiOn[0] = false;
    #endif
    #if HAS_MULTIPLE_ADDITIONAL_PSU > 1
      SET_OUTPUT(MULTIPSU_PSU2_PIN);
      OUT_WRITE(MULTIPSU_PSU2_PIN, HIGH);
      psuMultiOn[1] = false;
    #endif
  #endif
}
